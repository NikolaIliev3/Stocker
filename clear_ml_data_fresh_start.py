"""
Script to clear ML model and backtesting data for a fresh start
...
def clear_ml_data():
    ...
    # Also clear backtesting results
    files_to_remove.extend([
    ...
    if removed_count == 0:
        ...
        print(f"\n✓ Cleared {removed_count} ML model/backtesting file(s)")
        print("  Preserved: preferences.json, portfolio.json, predictions.json")
    
    print("\n✓ Fresh start ready! You can now train new models.")
...
if __name__ == "__main__":
    ...
    response = input("\nProceed with clearing ML data? (yes/no): ").strip().lower()
    if response in ['yes', 'y']:
        clear_ml_data()
    else:
        print("Cancelled. No data was cleared.")
