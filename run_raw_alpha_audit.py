...
logging.basicConfig(
    ...
    handlers=[
        ...
        logging.FileHandler("raw_audit_results.log")
    ]
)
logger = logging.getLogger('RawAudit')

# MONKEYPATCH: Bypass Elite Vetoes for Audit
def bypassed_pro_filters(self, final_action: str, final_confidence: float, 
    ...
    # Disable vetoes but keep some basic threshold for "meaningful" signals
    if final_action == 'BUY' and final_confidence > 60.0:
        return 'BUY', final_confidence
    elif final_action == 'SELL' and final_confidence > 60.0:
...
def run_audit_logic(app, root):
    try:
        ...
        logger.info(f"Generating {num_tests} test points for 2026 STRICT OOS window (RAW ALPHA)...")
        ...
        logger.info(f"Starting RAW ALPHA test phase on {len(test_points)} points...")
        
        active_predictions = 0
        
        for sym, t_date in test_points:
            # We use _test_historical_point which calls predictor.predict which calls _apply_pro_filters (now monkeypatched)
            ...
            if res and res.get('predicted_action') != 'HOLD':
                ...
                conf = res.get('predicted_confidence', 0)
                ...
                if active_predictions % 1 == 0:
                    logger.info(f"✓ Active Pred {active_predictions}: {sym} ({regime}) -> {action} at {conf:.1f}% -> {'CORRECT' if is_correct else 'WRONG'}")

        # Final Report
        logger.info(f"\n{'='*70}")
        logger.info(f"📊 FINAL RAW ALPHA PERFORMANCE REPORT (STRICT 2026 OOS)")
        logger.info(f"{'='*70}")
        ...
        logger.info(f"{'-'*70}")
        logger.info(f"OVERALL RAW PRECISION (>60% Threshold): {overall_precision:.1f}% ({total_correct}/{total_active})")
        logger.info(f"NOTE: Safety Vetoes were DISABLED for this audit to measure model capacity.")
        logger.info(f"{'='*70}\n")
        ...
        with open("raw_audit_summary.json", "w") as f:
            ...
            json.dump({
                ...
                "mode": "RAW ALPHA (Safety Vetoes Disabled)"
            }, f)
...
def run_large_audit():
    logger.info(f"\n{'='*70}")
    logger.info(f"🚀 STARTING RAW ALPHA AUDIT (300 SAMPLES - 2026 OOS)")
    logger.info(f"{'='*70}\n")
